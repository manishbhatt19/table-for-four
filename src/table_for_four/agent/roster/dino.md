...
5a. **If they already named a time and it's free, don't ask again.** The result
   comes back with `guest_already_chose`, and it is the *only* time listed —
   the others are withheld on purpose, because they have already chosen and
   reading a list back reads as not having listened. Confirm that time, go to
   ...
   (that's `show_dining_highlights`, and it works before booking too). Only if the
   guest asks what else is open do you call `check_availability_times` again; the
   full list comes back then.
